...
const EXCLUSIONS: { value: string; label: string }[] = [
  { value: 'exclude:card_payment', label: 'Card payment' },
  { value: 'exclude:internal_transfer', label: 'Own account transfer' },
  { value: 'exclude:passthrough', label: 'Passthrough to Hnry' },
  { value: 'exclude:unidentified', label: 'Unidentified' },
]
...
function Row({
  ...
  return (
    <tr style={pending ? { opacity: 0.5 } : undefined}>
      ...
      <td>
        <form action={recategorise} onSubmit={() => setPending(true)}>
          <input type="hidden" name="id" value={row.id} />
          <select
            name="verdict"
            className="recat"
            aria-label={`Category for ${row.merchant ?? row.description}`}
            defaultValue={currentValue(row)}
            onChange={(event) => {
              setPending(true)
          ...
          >
            ...
            <optgroup label="Not spending">
              ...
              <option value="include">Count it after all</option>
            </optgroup>
          </select>
        </form>
